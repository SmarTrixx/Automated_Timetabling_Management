...
//         const grouped = groupCalendarCells(allEntries, allTimeSlots, dayOrder);
//         autoTable(doc, {
//           startY: 20,
//           head: [['Day', ...allTimeSlots]],
//           body: dayOrder.map(day => {
//             const cells = [];
//             for (const cell of grouped[day]) {
//               cells.push(cell.courseStr);
//               for (
//                 let k = allTimeSlots.indexOf(cell.start) + 1;
//                 k <= allTimeSlots.indexOf(cell.end);
//                 k++
//               ) {
//                 cells.push('');
//               }
...
//           const grouped = groupCalendarCells(entries, allTimeSlots, dayOrder);
//           autoTable(doc, {
//             startY: 20,
//             head: [['Day', ...allTimeSlots]],
//             body: dayOrder.map(day => {
...
//   // Group consecutive slots for the same course for calendar view export
//   function groupCalendarCells(entries, allTimeSlots, dayOrder) {
//     const result = {};
//     for (const day of dayOrder) {
//       const dayEntries = [];
//       let i = 0;
//       while (i < allTimeSlots.length) {
//         const time = allTimeSlots[i];
//         const entry = entries.find(e => e.day === day && e.time === time);
//         if (!entry) {
//           dayEntries.push({ start: time, end: time, courseStr: '' });
//           i++;
//           continue;
//         }
//         // Start grouping consecutive slots for the same course
//         let j = i + 1;
//         while (
//           j < allTimeSlots.length &&
//           entries.find(
//             e =>
//               e.day === day &&
//               e.time === allTimeSlots[j] &&
//               e.course_code === entry.course_code &&
//               e.room === entry.room &&
//               e.instructor === entry.instructor
//           )
//         ) {
//           j++;
//         }
//         // j is now the first non-matching slot
//         const start = time;
//         const end = allTimeSlots[j - 1];
//         const courseStr = `${entry.course_code || ''}${entry.room ? ' (' + entry.room + ')' : ''}${entry.instructor ? '\n' + entry.instructor : ''}\n${start} - ${addHoursToTime(end, 1)}`;
//         dayEntries.push({ start, end, courseStr });
//         i = j;
//       }
//       result[day] = dayEntries;
//     }
//     return result;
//   }
...
//         disableExcel={viewMode === 'calendar' || viewMode === 'both'}
//       />
