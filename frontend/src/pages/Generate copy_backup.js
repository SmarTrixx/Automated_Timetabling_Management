    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      {(viewMode === 'calendar' || viewMode === 'both') && timetable && timetable.schedule && Object.keys(currentSchedule).length > 0 && (
        <div id="calendar-print-area">
          {fullView ? (
            <div ref={el => calendarRefs.current['faculty'] = el} className="calendar-level-print">
            ...
            Object.entries(currentSchedule).map(([level, entries]) => (
              <div key={level} ref={el => calendarRefs.current[level] = el} className="calendar-level-print">
                <TimetableCalendarView
                  fullView={false}
                  allEntries={allEntries}
                  currentSchedule={{ [level]: entries }}
                  allTimeSlots={allTimeSlots}
                  dayOrder={dayOrder}
                  getCellEntry={getCellEntry}
                  getConflictedCells={getConflictedCells}
